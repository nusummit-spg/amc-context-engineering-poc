...
from app.engine.extraction_prompts import DOCX_IMAGE_PROMPT, PPTX_SLIDE_IMAGE_PROMPT
...
def _describe_docx_images(doc, use_vision: bool = True) -> str:
    ...
    for rel in doc.part.rels.values():
        ...
        try:
            ...
            desc = extract_from_image(img_bytes, DOCX_IMAGE_PROMPT, model=config.GROQ_VISION_MODEL)
            if desc:
...
def extract_csv_text_full(path: str, rows_per_page: int = 25, max_pages: int = 50) -> List[Dict[str, Any]]:
    ...
    for i in range(0, len(body), rows_per_page):
        ...
        lines = list(header_md) + ["| " + " | ".join(r) + " |" for r in batch]
        pages.append({
            "page_num": page_num,
            "text": "\n".join(lines),
