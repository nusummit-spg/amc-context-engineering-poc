...
class GraphTraversal:
    ...
    async def traverse(
        ...
        return await strategy(entities)

    # ---------- strategies ----------

    async def _exposure(self, entities: list[BaseEntity]):
        facts, paths, cyphers = [], [], []
        ...
        if not groups:
            for issuer in (e for e in entities if e.entity_type == EntityType.ISSUER):
                ...
                for row in rows:
                    groups.append(BaseEntity(
                        entity_type=EntityType.ISSUER_GROUP, name=row["name"],
                    ))
    ...
    async def _compliance(self, entities: list[BaseEntity]):
        facts, paths, cyphers = [], [], []
        ...
        if circulars:
            for circ in circulars:
                rows += await self._graph.run(cql.CIRCULAR_AFFECTED_SCHEMES, circular_name=circ.name)
                cyphers.append("CIRCULAR_AFFECTED_SCHEMES")
        elif clauses:
            for clause in clauses:
                rows += await self._graph.run(cql.CLAUSE_AFFECTED_SCHEMES, clause_name=clause.name)
    ...
    async def _house_view(self, entities: list[BaseEntity]):
        facts, paths, cyphers = [], [], []
    ...
    async def _entity_lookup(self, entities: list[BaseEntity]):
        facts, paths, cyphers = [], [], []
    ...
    async def _noop(self, entities: list[BaseEntity]):
        return [], [], []
